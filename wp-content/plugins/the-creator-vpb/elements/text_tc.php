...
function tcvpb_text_tc_shortcode( $attributes, $content = null ) {
	...
	$classes=array();
	if($animation!='none')      $classes[] = 'tcvpb-animo';
	if($class!='')    $classes[] = $class;
	if($desktop==1)    $classes[] = 'hidden-desktop';
	if($tablet==1)     $classes[] = 'hidden-tablet';
	if($phablet==1)    $classes[] = 'hidden-phablet';
	if($phone==1)      $classes[] = 'hidden-phone';
	...
	$animation_out = ($animation!='none') ? 'data-animation="'.esc_attr($animation).'" data-duration="'.esc_attr($duration).'" data-delay="'.esc_attr($delay).'"' : '';

	if ( $members==1 && is_user_logged_in() && !is_null( $content ) && !is_feed() ){
		return '';
	}
	elseif ( $non_members==1 && !is_user_logged_in() && !is_null( $content ) && !is_feed() ) {
		return '';
	}
	else{
		return '<div '.esc_attr($id_out).' class="'.esc_attr($classes).'" '.$animation_out.'><p>'.do_shortcode($content).'</p></div>';
	}

...
function tcvpb_image_hotspots_tc_shortcode( $attributes, $content = null ) {
	...
	return '<div '.esc_attr($id_out).' class="tcvpb-animo '.esc_attr($class).'" data-animation="'.esc_attr($animation).'" data-duration="'.esc_attr($duration).'" data-delay="'.esc_attr($delay).'" style="position:relative;"><img src="'.esc_url($url).'">'.  do_shortcode($content)  . '</div>';
}
...
function tcvpb_image_hotspot_tc_shortcode( $attributes, $content = null ) {
	...
	$return = '
		<div class="tcvpb-hotspot-body" style="position:absolute; top:'.esc_attr($top).'%; left:'.esc_attr($left).'%;">
			<a class="tcvpb-hotspot-tooltip" style="'.esc_attr($hotspot_background.$color).'" title="'.do_shortcode($content).'" data-gravity="'.esc_attr($gravity).'">
				'.$icon_out.'
				'.$number_out.'
			</a>

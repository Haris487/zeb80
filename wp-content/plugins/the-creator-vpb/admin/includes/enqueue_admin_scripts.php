...
function tcvpb_enqueue_admin_scripts($hook) {
	...
	$fonts = array('typicons','whhg','big_mug','entypo','font_awesome','google_material','icomoon','ionicon');
	...
	foreach ($tcvpb_elements as $name => $element) {
		...
		if(isset($element['type']) && $element['type']=='block' && empty($element['hidden'])){
			$element_categories[$element['category']] = $element['category'];
		}
	}
	$element_categories=array_values($element_categories);
	
	wp_enqueue_style('tcvpb-spectrum-css', TCVPB_DIR.'admin/css/spectrum.css', array(), TCVPB_VERSION);
	wp_enqueue_style('tcvpb-admin-icons', TCVPB_DIR.'admin/css/pluginicons/pluginicons.css', array(), TCVPB_VERSION);
	wp_enqueue_style('tcvpb-admin-style', TCVPB_DIR.'admin/css/admin.css', array('tcvpb-spectrum-css','tcvpb-admin-icons'), TCVPB_VERSION);
	wp_enqueue_media();
	wp_enqueue_script('tcvpb-spectrum', TCVPB_DIR.'admin/js/spectrum.js', array(), TCVPB_VERSION,true);
	wp_enqueue_script('tcvpb-admin-script', TCVPB_DIR.'admin/js/admin.js', array('tcvpb-spectrum','jquery-ui-sortable','jquery-ui-resizable','jquery-ui-draggable'), TCVPB_VERSION,true);
	wp_localize_script('tcvpb-admin-script', 'tcvpb_from_WP', array(
		...
		'rearange_sections' => __('Rearange Sections', 'the-creator-vpb'),
